    ...
    switch (request.getRequestType()) {
      ...
      case STOP_SESSION:
        ...
        if (request.getRequestType() == RequestType.STOP_SESSION) {
          log.fine("Got request to stop session , Video recording flag is = "+session.getVideoRecordingFlag());
          if(session.getVideoRecordingFlag()){
            log.info("Got Request to stop video Recording");
            session.stopVideoRecording(response);
          }
          if(session.getNetworkLoggingFlag()){
            log.info("Got Request to stop video Recording");
            session.stopNetworkLogging();
          }        
        ...
        if(request.getRequestType() == RequestType.STOP_SESSION)
          registry.terminate(session, SessionTerminationReason.CLIENT_STOPPED_SESSION);   
        break;
      case NETWORK_LOG_CAPTURE:
        try{
          session = getSession();
          log.fine("Session is = "+session.toString());
          session.handleNetowrkLogRequests(request,response);
          log.info("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ Finally Moved to Request Capturing To be Implemented $$$$$$$$$$$$$$$$$$$$$$$ ");
        }catch(Exception e){log.fine("Exception with Network work related requests");e.printStackTrace();}
      break;
      case GET_MONITOR_STATS:
        
        log.info("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ Finally Moved to Monitor Stats To be Implemented $$$$$$$$$$$$$$$$$$$$$$$ ");
      break;
       default:  
      break;
    }

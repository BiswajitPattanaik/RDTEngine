...
public class TestSession {
  ...
  //TO:DONE changes done by biswajit for video recording .
  //Value when video recording is in continue.
  private final boolean VIDEO_RECORDING_CONTINUE = true;
  //Value When video recording in not runing . Also the default value of Video Recording Flag .
  private final boolean VIDEO_RECORDING_NOT_CONTINUE = false;
  private final String WORKING_DIR_NAME = "WORKING_DIR";
  private boolean videoRecordingFlag = VIDEO_RECORDING_NOT_CONTINUE;
  private VideoRecorder videoRecorder = null ;
  private NetworkLoggerHandler networkLoggerHandler = null ;
  private boolean NETWORk_LOGGING_ACTIVE = true ;
  private String currentWorkingDir = null;
  //Default value for network logging .
  private boolean NETWORK_LOGGING_INACTIVE = false;
  private boolean networkLogging = NETWORK_LOGGING_INACTIVE ; 
  ...
  public TestSession(
      ...
      try{
        ...
        else if (isVideoRecordingEnabled && !platformName.equalsIgnoreCase("Android")){
          log.fine("Session Can not be started as platoformName is not Android");
        }
        else{
        }
     
      log.fine("Hurrey New Session Has been started session capabilities ="+requestedCapabilities+" External Key is = "+externalKey);
    }catch(Exception e){log.fine("Exception Occured hence exiting");e.printStackTrace();}
  }
...
//## Separating Control flow for RDTBased session and Non Rdt based session 
  ...
  private void setThreadDisplayName() {
    ...
    log.fine("Current Thread name = "+name+"**********");
  }
  ...
  private HttpRequest prepareProxyRequest(HttpServletRequest request) throws IOException {
    ...
    log.info(String.format("remote url is = %s , ServletPath = %s , ContextPath = %s , extra Path info = %s, path = %s , requestBody = %s ",remoteURL,request.getServletPath(),request.getContextPath(),request.getPathInfo(),request.getRequestURI(),requestBody));
  ...
  public void setVideoRecordingFlag(boolean flag){
    videoRecordingFlag=flag;
  }

  public boolean getVideoRecordingFlag(){
    return videoRecordingFlag;
  }
  public void startVideoRecording(){
    try{
      videoRecorder.startRecording();  
      setVideoRecordingFlag(VIDEO_RECORDING_CONTINUE);}
      catch(Exception e){e.printStackTrace();}
  }
  public void stopVideoRecording(HttpServletResponse response){
    try{
    videoRecorder.stopRecording(response);
    setVideoRecordingFlag(VIDEO_RECORDING_NOT_CONTINUE);}
    catch(Exception e){e.printStackTrace();}
  }
  public void stopNetworkLogging(){
    try{
      networkLoggerHandler.stopNetworkLogger();
    }catch(Exception e){e.printStackTrace();}
  }
  private String createWorkingDir() throws Exception{
    ...
    //try{
      ...
      if (!workingDir.exists()){
        ...
        createSessionWorkingDir(workingDirPath);
      }
      else{
        createSessionWorkingDir(workingDirPath);  
      }
  ...
  public void createSessionWorkingDir(String workingDirPath)throws Exception{
    ...
    boolean result = sessionWorkingDir.mkdir();
      if (!result){
        throw new Exception("Not able to Create Session Working Dir hence existing");
      }
    }
  }
  public boolean getNetworkLoggingFlag(){
    return networkLogging;
  }
  public void setNetworkLoggingFlag(boolean flag){
    networkLogging=flag;
  }
  public void handleNetowrkLogRequests(SeleniumBasedRequest request , HttpServletResponse response)throws Exception {
    URL remoteURL = slot.getRemoteURL();
    String requestBody = null;
    if ("POST".equalsIgnoreCase(request.getMethod())) {
      Scanner s = new Scanner(request.getInputStream(), "UTF-8").useDelimiter("\\A");

...
public class JsonHttpResponseCodec extends AbstractHttpResponseCodec {
  ...
  @Override
  protected Response reconstructValue(Response response,HttpResponse encodedResponse) {
    try {
      errorHandler.throwIfResponseFailed(response, 0);
    } catch (Exception e) {
      response.setValue(e);
    ...
    //response.setValue(elementConverter.apply(response.getValue()));
    response.setValue(encodedResponse.getContentString().trim());
    response.setByteArrayResponse(encodedResponse.getContent());
    //System.out.println("Inside Json Http Response Codec byte content ="+encodedResponse.getContent()); 
    return response;
  }
  @Override

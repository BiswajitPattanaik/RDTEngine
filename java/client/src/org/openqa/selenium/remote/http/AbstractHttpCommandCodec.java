...
import static org.openqa.selenium.remote.DriverCommand.NETWORK_LOG_CAPTURE;
import static org.openqa.selenium.remote.DriverCommand.GET_MONITOR_STATS;
...
public abstract class AbstractHttpCommandCodec implements CommandCodec<HttpRequest> {
  ...
  public AbstractHttpCommandCodec() {
    ...
    defineCommand(NETWORK_LOG_CAPTURE, post("/session/:sessionId/networklog"));
    defineCommand(GET_MONITOR_STATS, post("/session/:sessionId/monitorstats"));
  }

  @Override
  public HttpRequest encode(Command command) {
    String name = aliases.getOrDefault(command.getName(), command.getName());
    //System.out.println(" [DEBUG] "+String.format("name = %s , Command Name = %s ",name,command.getName()));
    CommandSpec spec = nameToSpec.get(name);
    //System.out.println(" [DEBUG] "+String.format("spec = %s ",spec));
    if (spec == null) {
  ...
  protected void defineCommand(String name, CommandSpec spec) {
    //System.out.println("[ DEBUG ] Command Spec Initiated for String Name ="+name);
    checkNotNull(name, "null name");
    //System.out.println("[ DEBUG ] Command Spec = "+spec+" String Name ="+name);
  ...
  private String buildUri(
    ...
    //System.out.println("[DEBUG] Path segments "+spec.pathSegments);

...
public abstract class AbstractHttpResponseCodec implements ResponseCodec<HttpResponse> {
    ...
    try {
      ...
      if (contentType.startsWith("application/json")) {
        if (content.isEmpty()) {
          ...
          //System.out.println("Inside Abstract Response Codec content type returning Empty Response");
          return new Response();
        }
        //System.out.println("Inside Abstract Response Codec content type returning new Exception can not decode");
        throw new IllegalArgumentException(
            "Cannot decode response content: " + content, e);
      }
    }
  ...
  protected abstract Response reconstructValue(Response response,HttpResponse encodedResponse);
}
